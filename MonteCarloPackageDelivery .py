...
# Function for cost and volume check!!!s


def volume_check(Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels, Number_of_20_Kg_Parcels,
                 Dimensions_of_all_size_boxes_cm, cargo_carrier_dimensions_cm):
    """
    ...
    if (volume_1_kg + volume_5_kg + volume_10_kg + volume_20_kg) <= (volume_cargo_carrier * 0.85) or (
            volume_1_kg + volume_5_kg + volume_10_kg + volume_20_kg) >= volume_cargo_carrier:
        boolean_y = False
    return boolean_y
...
def random_sampling(cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm):
    ...
    False
    >>> len(random_sampling([150,150,150],{"1kg":[5,5,5],"5kg":[15,15,15],"10kg":[25,25,25],"20kg":[35,35,35]}))
    4
    """
    ...
    if volume_check(Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels, Number_of_20_Kg_Parcels,
                    Dimensions_of_all_size_boxes_cm, cargo_carrier_dimensions_cm):
...
def load_unloading_time_calculation(Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels,
                                    Number_of_20_Kg_Parcels):
    """
    ...
    False
    >>> len(load_unloading_time_calculation(25,25,65,75))==2
    True
    """

    Number_of_workers_needed_to_carry = {"1_kg": 1, "5_kg": 1, "10_kg": 2, "20_kg": 2}
    Number_of_Parcels_list = {"1_kg": Number_of_1_Kg_Parcels, "5_kg": Number_of_5_Kg_Parcels,
                              "10_kg": Number_of_10_Kg_Parcels, "20_kg": Number_of_20_Kg_Parcels}
    ...
    while Number_of_Parcels_list["1_kg"] > 0 or Number_of_Parcels_list["5_kg"] > 0 or Number_of_Parcels_list[
        "10_kg"] > 0 or Number_of_Parcels_list["20_kg"] > 0:
        Number_of_workers_available = Number_of_workers
        Needed_workers = 0
        while Number_of_workers_available > 0:
            ...
            if Number_of_Parcels_list["1_kg"] == 0 and Number_of_Parcels_list["5_kg"] == 0 and Number_of_Parcels_list[
                "10_kg"] == 0 and Number_of_Parcels_list["20_kg"] == 0:
                break
        Time_takes += 5
...
def Hypothesis_1(cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm):
    ...
    Number_of_worker = []
    Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels, Number_of_20_Kg_Parcels = random_sampling(
        cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm)

    for i in range(100):
        Time_takes, Number_of_workers = load_unloading_time_calculation(Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels,
                                                                        Number_of_10_Kg_Parcels,
                                                                        Number_of_20_Kg_Parcels)
    ...
    Hypothesis_1 = pd.DataFrame(
        data={"Number_of_10&20_kg": Number_of_10_and_20_kg, "loading_and_unloading_time": loading_and_unloading_time,
...
def cost_for_model(total_cost, a, b, c, d):
    ...
    True
    >>> cost_for_model(1000,1,1,1,1) < -30
    False
    >>> cost_for_model(1000,1,1,1,1) <0
    False
    """
    ...
    total_cost1 = total_cost + a * model_costs['1kg'] + b * model_costs['5kg'] + c * model_costs['10kg'] + d * \
                  model_costs['20kg']

    return total_cost1


def before(day_count, cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm):
    """
    ...
    >>> before(140,[150,150,150],{"1kg":[5,5,5],"5kg":[15,15,15],"10kg":[25,25,25],"20kg":[35,35,35]}) < 0.1
    False
    >>> before(140,[150,150,150],{"1kg":[5,5,5],"5kg":[15,15,15],"10kg":[25,25,25],"20kg":[35,35,35]}) > 100
    False
    >>> len(str(int(before(140,[150,150,150],{"1kg":[5,5,5],"5kg":[15,15,15],"10kg":[25,25,25],"20kg":[35,35,35]}))))
    1
    """
    ...
    for i in range(int((1 - percent_selection_for_no_delivery_days) * day_count)):
        ...
        Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels, Number_of_20_Kg_Parcels = random_sampling(
            cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm)
        Number_of_parcels_not_delivered_in_time += int((
                                                                   Number_of_1_Kg_Parcels + Number_of_5_Kg_Parcels + Number_of_10_Kg_Parcels + Number_of_20_Kg_Parcels) * percent_selection)
        Number_of_parcels_delivered_in_time_on_bad_weather_days += int(
            (Number_of_1_Kg_Parcels + Number_of_5_Kg_Parcels + Number_of_10_Kg_Parcels + Number_of_20_Kg_Parcels) * (
                        1 - percent_selection))
    ...
    success_rate = (Number_of_parcels_delivered_in_time + Number_of_parcels_delivered_in_time_on_bad_weather_days) / (
                Number_of_parcels_delivered_in_time + Number_of_parcels_not_delivered_in_time + Number_of_parcels_delivered_in_time_on_bad_weather_days + Number_of_parcels_totally_not_delivered_in_time)
    return success_rate


# ### Implementation of self driving cars and drone-delivery at extreme weather

def after(day_count, cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm):
    """
    ...
    >>> after(140,[150,150,150],{"1kg":[5,5,5],"5kg":[15,15,15],"10kg":[25,25,25],"20kg":[35,35,35]}) > (0.2, 0)
    True
    >>> len(after(140,[150,150,150],{"1kg":[5,5,5],"5kg":[15,15,15],"10kg":[25,25,25],"20kg":[35,35,35]}))
    2
    >>> after(140,[150,150,150],{"1kg":[5,5,5],"5kg":[15,15,15],"10kg":[25,25,25],"20kg":[35,35,35]}) > (100,0)
    False
    """
    Number_of_parcels_not_delivered_in_time = 0
    Number_of_parcels_delivered_in_time = 0
    ...
    for i in range(Number_of_bad_weather_days_delivery_expected):
        Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels, Number_of_20_Kg_Parcels = random_sampling(
            cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm)
        Total_cost = cost_for_model(Total_cost, Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels,
                                    Number_of_20_Kg_Parcels)
    for i in range(Number_of_bad_weather_delivery_fails):
        Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels, Number_of_20_Kg_Parcels = random_sampling(
            cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm)
        Number_of_parcels_not_delivered_in_time += (
                    Number_of_1_Kg_Parcels + Number_of_5_Kg_Parcels + Number_of_10_Kg_Parcels + Number_of_20_Kg_Parcels)
    for i in range(365 - Number_of_bad_weather_delivery_fails):
        Number_of_1_Kg_Parcels, Number_of_5_Kg_Parcels, Number_of_10_Kg_Parcels, Number_of_20_Kg_Parcels = random_sampling(
            cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm)
        Number_of_parcels_delivered_in_time += Number_of_1_Kg_Parcels + Number_of_5_Kg_Parcels + Number_of_10_Kg_Parcels + Number_of_20_Kg_Parcels
    success_rate = Number_of_parcels_delivered_in_time / (
                Number_of_parcels_delivered_in_time + Number_of_parcels_not_delivered_in_time)
...
def Hypothesis_2(cargo_carrier_dimensions_cm,Dimensions_of_all_size_boxes_cm):
    ...
    for i in range(0, 120, 5):
        sucess_rate_without_implementing.append(before(i, cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm))
        sucess_rate_with_implementing.append(after(i, cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm))
        Number_of_days.append(i)
    sucess_rate_without = pd.DataFrame(
        data={"Before": sucess_rate_without_implementing, "Number_of_days": Number_of_days})
    ...
    Diff_of_before_after_sucess_rates = pd.DataFrame(
        data={"Difference": (sucess_rate_with["After"] - sucess_rate_without["Before"]),
              "Number_of_days": Number_of_days})
    model_database = pd.merge(left=sucess_rate_without, right=sucess_rate_with, how='inner', left_on='Number_of_days',
                              right_on='Number_of_days')
...
def cost_of_Self_driving_Car(model_database):
    ...
    >>> model_database = pd.DataFrame(columns=['Before', 'Number_of_days','After','Total_cost'])
    >>> model_database = model_database.append(pd.Series({'Before':1,'Number_of_days':0,'After':1,'Total_cost':0}), ignore_index=True)
    >>> cost_of_Self_driving_Car(model_database)
      Before Number_of_days After Total_cost self_driving_cost_of_maintenance
    0      1              0     1          0                          -100000
    >>> len(cost_of_Self_driving_Car(model_database))
    1
    >>> max(cost_of_Self_driving_Car(model_database))
    'self_driving_cost_of_maintenance'
    >>> min(cost_of_Self_driving_Car(model_database))
    'After'
    """
    cost_of_Self_driving_Car = 100000
    Number_of_cars = 1
    model_database["self_driving_cost_of_maintenance"] = model_database["Total_cost"] - (
                cost_of_Self_driving_Car * Number_of_cars)
...
def cost_of_drone(model_database):
    ...
    >>> model_database = pd.DataFrame(columns=['Before', 'Number_of_days','After','Total_cost','self_driving_cost_of_maintenance'])
    >>> model_database = model_database.append(pd.Series({'Before':1,'Number_of_days':0,'After':1,'Total_cost':0,'self_driving_cost_of_maintenance':100000}), ignore_index=True)
    >>> cost_of_drone(model_database)
      Before  ... drone_cost_of_maintenance
    0      1  ...                    -35000
    <BLANKLINE>
    [1 rows x 6 columns]

    >>> len(cost_of_drone(model_database))
    1
    >>> max(cost_of_drone(model_database))
    'self_driving_cost_of_maintenance'
    >>> min(cost_of_drone(model_database))
    'After'
    """
    cost_of_drone = 400
    maintenance_of_drone = 25 * 12
    number_of_drones = 50
    model_database["drone_cost_of_maintenance"] = model_database["Total_cost"] - (
                (cost_of_drone + maintenance_of_drone) * number_of_drones)
    return model_database
...
if __name__ == '__main__':
    ...
    Hypothesis_1_result = Hypothesis_1(cargo_carrier_dimensions_cm, Dimensions_of_all_size_boxes_cm)
